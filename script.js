...
const terminalTips = ["Type 'help' for commands."];
...
const fileSystem = {
  ...
  projects: {
    parent: "root",
    directories: [],
    files: ["Vault-F", "Cloak-F", "Shredder-F", "Checkmate-F", "Stencil-F"],
  },
...
const fileData = {
  "contact.txt":
    "Email: hello@frqise.me\nGithub: @frqise\nTwitter: @its_FrQise",
  "readme.md": "# Documents\nTest Readme File",
  "01_entry.txt": `DATE: 2026-01-01\nTITLE: First Entry\n-------------------\nThis is an example journal entry.`,
  "chef.txt": "Go to the kebab, call them Chef, enjoy your meal.",
  "Vault-F":
    "A lightweight, local-first file password manager utilizing AES-256 and PBKDF2.\nhttps://github.com/FrQise/Vault-F",
  "Cloak-F":
    " A lightweight, local-first file encryption and compression utility utilizing AES-256 and PBKDF2.\nhttps://github.com/FrQise/Cloak-F",
  "Shredder-F":
    " A lightweight, local-first secure data eraser & disk sanitizer\nhttps://github.com/FrQise/Shredder-F",
  "Checkmate-F":
    " A lightweight, local-first forensic file integrity & directory auditing program.\nhttps://github.com/FrQise/Checkmate-F",
  "Stencil-F":
    "A lightweight, local-first steganographic protection & AI-poisoning utility for digital media.\nhttps://github.com/FrQise/Stencil-F",
};
...
function showRandomTip() {
  tipDisplay.textContent =
    terminalTips[Math.floor(Math.random() * terminalTips.length)];
}
showRandomTip();
...
input.addEventListener("input", () => {
  ...
  if (parts.length === 1) {
    ...
    const items = [
      ...fileSystem[currentPath].directories,
...
<span class="dim-text">Permissions:</span> Read-only (Sudo restricted)
    ...
    case "pwd":
      output =
        currentPath === "root" ? "/home/guest" : `/home/guest/${currentPath}`;
      break;
...
<div class="fetch-container">
    ...
    case "ls":
      ...
      output =
        dir.directories
          .map((d) => `<span class="dir-text">${d}/</span>`)
          .join("  ") +
        "  " +
        dir.files.join("  ");
      break;
    ...
    case "cd":
      if (!arg || arg === "~") currentPath = "root";
        ...
        currentPath = fileSystem[currentPath].parent || "root";
  ...
  outputBuffer.innerHTML +=
    echo + `<div class="command-output">${output}</div>`;
